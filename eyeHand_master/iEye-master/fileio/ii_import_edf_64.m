...
if isequal(filename,0)
    ...
    % EXTRACT SAMPLES
    [status,result] = system(['edf2asc_x86_64 -t -c -s -miss 0 ' fullfile(pathname, filename)]);
    disp(status);
    disp(result);
    ...
    % EXTRACT EVENTS
    [status,result] = system(['edf2asc_x86_64 -t -c -e -miss 0 ' fullfile(pathname, filename)]);
    disp(status);
    disp(result);
