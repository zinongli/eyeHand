function [ ii_data, ii_cfg ] = rsvp_prf_driftcorrect( ii_data, ii_cfg, chan_names, correct_mode, target_coords, rsvp_params )
...
% RSVP edit - JK
bounds_vert_res = split(rsvp_params.stim_bounds, ',');
bounds_vert_res = str2num(bounds_vert_res{2});
max_adj = bounds_vert_res / (rsvp_params.ppd * 2);
...
for cc = 1:length(chan_names)
    ...
    for sel_idx = 1:size(ii_cfg.cursel,1)
        ...
        % RSVP edit
        % Sometimes a bad fixation in ITI can mess up adjustments. Lets fix
        % that by replacing adjustment amount if the end result is off
        
        %{
        if abs(nanmean(ii_data.(tr_idx)) - adj_by) > max_adj || abs(nanmean(ii_data.Y(tr_idx)) - adj_by) > max_adj
            adj_by = NaN;
        else
        %}
        
        if abs(nanmean(ii_data.(chan_names{cc})(tr_idx)) - adj_by) > max_adj
            adj_by = NaN;
        else
...
% RSVP edit - JK
% If there is no fixation in ITI to use for calibration, don't just ignore
% it - instead use avg calibration. Ideally subjects would be fixating
% throughout, but they tend to purposefully blink during the ITI 
for i = 1:length(chan_names)
    for j = 1:length(ii_cfg.drift.amt)
        if isnan(ii_cfg.drift.amt(j,i))
            ii_cfg.drift.amt(j,i) = nanmean(ii_cfg.drift.amt(:,i));
            tr_idx = ii_cfg.trialvec == j;
            chans_to_adjust = { all_fields{ cellfun(@(x) any(x) && x(1)==1 , strfind(all_fields,chan_names{i} )) } };
            for ca = 1:length(chans_to_adjust)
                ii_data.(chans_to_adjust{ca})(tr_idx) = ii_data.(chans_to_adjust{ca})(tr_idx)- ii_cfg.drift.amt(j,i);
            end
            clear tr_idx;
        end
    end
...
%{
new_adj = false;
for i = 1:length(ii_cfg.drift.amt)
    if isnan(ii_cfg.drift.amt(i,1))
        ii_cfg.drift.amt(i,1) = nanmean(ii_cfg.drift.amt(:,1));
        tr_idx = ii_cfg.trialvec == i;
        new_adj = true;
    end
    if isnan(ii_cfg.drift.amt(i,2))
        ii_cfg.drift.amt(i,2) = nanmean(ii_cfg.drift.amt(:,2));
        tr_idx = ii_cfg.trialvec == i;
        new_adj = true;
    end
    if new_adj
        for j = 1:length(chan_names)
            chans_to_adjust = { all_fields{ cellfun(@(x) any(x) && x(1)==1 , strfind(all_fields,chan_names{j} )) } };
            for ca = 1:length(chans_to_adjust)
                ii_data.(chans_to_adjust{ca})(tr_idx) = ii_data.(chans_to_adjust{ca})(tr_idx)- ii_cfg.drift.amt(i,j);
            end
        end
        clear tr_idx;
    end
