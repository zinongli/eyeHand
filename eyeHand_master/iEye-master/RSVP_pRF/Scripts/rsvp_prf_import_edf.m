function [ii_data,ii_cfg] = rsvp_prf_import_edf(edf_file,ifg_file,data_file, varargin)
...
[status,result] = system(['/usr/local/bin/edf2asc -t -c -e -miss 0 ' edf_file]);
...
% Edit: in pRF eyetracking analysis, very infrequently, an xDAT 2 may print before the trial begins. The
% following code should handle that.
for i = 1:(length(remain)-1)
    if strcmp(varbl(i),'xDAT')
        if strcmp(remain(i), remain(i+1))
            % Swap positions of eyetracker  messages
            tmp = varbl(i+1);
            varbl(i+1) = varbl(i+2);
            varbl(i+2) = varbl(i+3);
            varbl(i+3) = tmp;
            ...
            vval(i+3) = tmp;
            tmp = samp_n(i+1);
            samp_n(i+1) = samp_n(i+2);
            samp_n(i+2) = samp_n(i+3);
            samp_n(i+3) = tmp;
        end
    end
end
