function [ii_data,ii_cfg,ii_sacc] = rsvp_prf_preproc(edf_fn,cfg_fn,preproc_fn,ii_params,trialinfo,skip_steps)
...
% import data
[ii_data,ii_cfg] = rsvp_prf_import_edf(edf_fn,cfg_fn,[edf_fn(1:end-4) '_iEye.mat']);
...
[ii_data,ii_cfg] = ii_censorchans(ii_data,ii_cfg,{'X','Y'},...
    {[-0.75 0.75]*ii_params.resolution(1)/ii_params.ppd,...
     [-0.75 0.75]*ii_params.resolution(2)/ii_params.ppd});
...
% 'raw' or 'smoothed' data on each channel)
    [ii_data,ii_cfg] = rsvp_prf_driftcorrect(ii_data,ii_cfg,{'X','Y'},ii_params.drift_select_mode,...
        ii_params.drift_target, rsvp_params);
end
...
if ~ismember('calibration',skip_steps)
    ...
    if ismember(ii_params.calibrate_mode,{'scale','rotate'})
        ...
        [ii_data,ii_cfg] = rsvp_prf_calibratebytrial(ii_data,ii_cfg,{'X','Y'},...
...
% Edit for RSVP - JK
[ii_cfg, ii_sacc] = rsvp_prf_proc(ii_data, ii_cfg, ii_sacc);


% plot all these - make sure they're good
f_all = ii_plotalltrials(ii_data,ii_cfg,{'X','Y'},[],[],ii_params.epoch_chan,ii_params.plot_epoch,0);
...
% Edit for RSVP - JK
% Use the rsvp_prf_plottimeseries function, which also includes info about
% where the stimuli are presented on the screen, and some other helpful
% features
if ii_params.show_plots == 1
    f_ts = rsvp_prf_plottimeseries(ii_data,ii_cfg,{'X','Y','TarX','TarY'});
else
    f_ts = rsvp_prf_plottimeseries(ii_data,ii_cfg,{'X','Y','TarX','TarY'},'nofigure');
end
