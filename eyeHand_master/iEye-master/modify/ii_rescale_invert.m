function [ii_data,ii_cfg] = ii_rescale_invert(ii_data, ii_cfg, chan, resolution, ppd, fixation)
% created by Mrugank (11/01/2022): created by combining ii_rescale and
% ii_invert. 
% ii_rescale_invert rescales channel values to known units (e.g., pix to dva)
% However, the dva is now saved as independent variable and does not
% replace x and y pixx values. Simultaneously, the x and y are inverted
% similar to ii_invert.
% Used for converting 'raw' gaze coordinates into the coordinate system
% used to present stimuli. 
% [ii_data,ii_cfg] = ii_rescale_invert(ii_data,ii_cfg,chan,resolution) rescales channels
